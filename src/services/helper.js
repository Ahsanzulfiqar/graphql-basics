...
const OnBoarding_Mail = (params) => {
  ...
  const templatePath = path.join(__dirname, "../assets/templates/leanerOnBoarding.html");
  ...
  fs.readFile(
    ...
    function (error, html) {
      if (error) {
        console.error(new Error(error, "Reading html template - OnBoarding_Mail"));
        throw err;
      } else {
        ...
        transporter.sendMail(mailOptions, function (error, info) {
          if (error) {
            console.log(error, );
            console.log(new Error(error, "Sending html template - OnBoarding_Mail"));
          } else {
